        ...
        for i, data in enumerate(loader):
            ...
            if args.loss == 'ce':
                if (args.model == 'dsmil' and args.ds_average) or (args.model == 'mhim' and isinstance(test_logits,(list,tuple))) or (args.model == 'pure' and args.baseline == 'dsmil'):
                    ...
                    if batch_size > 1:
                        bag_logits.extend(torch.softmax(test_logits,dim=-1)[:,1].cpu().squeeze().numpy())
                    else:
                        bag_logits.append(torch.softmax(test_logits,dim=-1)[:,1].cpu().squeeze().numpy())
            elif args.loss == 'bce':
...
if __name__ == '__main__':
    ...
    # Dataset 
    parser.add_argument('--datasets', default='gc', type=str, help='[camelyon16, tcga, gc]')
    parser.add_argument('--dataset_root', default='', type=str, help='Dataset root path')
    parser.add_argument('--label_path', default='', type=str, help='label of train dataset')
