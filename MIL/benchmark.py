...
def run_script(mil):
    if mil == 'ab':
        ...
        os.system(f'CUDA_VISIBLE_DEVICES={CPU_ID1}, python3 mil.py --project={PROJECT_NAME} --dataset_root={DATASET_PATH} --label_path={LABEL_PATH} --model_path={OUTPUT_PATH} --datasets={DATASET} --input_dim={NUM_DIM} --cv_fold=1 --title={TITLE_NAME} --model=pure --baseline=attn --train_val --seed={SEED} --wandb')
        ...
        os.system(f'CUDA_VISIBLE_DEVICES={CPU_ID2}, python3 mil.py --project={PROJECT_NAME} --dataset_root={DATASET_PATH} --label_path={LABEL_PATH} --model_path={OUTPUT_PATH} --datasets={DATASET} --input_dim={NUM_DIM} --cv_fold=1 --title={TITLE_NAME} --model=pure --baseline=selfattn --train_val --seed={SEED} --wandb')

        # TITLE_NAME=f'{FEATURE_NAME}-mhim\(transmil\)-{DATASET}-trainval-{SERVER}-{SEED}'
        # TEACHER_INIT=f'./{OUTPUT_PATH}/{PROJECT_NAME}/{FEATURE_NAME}-transmil-{DATASET}-trainval-{SERVER}-{SEED}'
        # os.system(f'CUDA_VISIBLE_DEVICES={CPU_ID2}, python3 mil.py --project={PROJECT_NAME} --dataset_root={DATASET_PATH} --model_path={OUTPUT_PATH} --label_path={LABEL_PATH} --datasets={DATASET} --cv_fold=1 --input_dim={NUM_DIM} --train_val --teacher_init={TEACHER_INIT} --model=mhim --baseline=selfattn --mask_ratio_h=0.03 --mask_ratio_hr=0.5 --mrh_sche --title={TITLE_NAME} --mask_ratio=0. --mask_ratio_l=0.8 --cl_alpha=0.1 --mm_sche --init_stu_type=fc --attn_layer=0 --seed={SEED} --wandb')
    elif mil == 'dsmil':
        TITLE_NAME=f'{FEATURE_NAME}-dsmil-{DATASET}-trainval-{SERVER}-{SEED}'
        os.system(f'CUDA_VISIBLE_DEVICES={CPU_ID1}, python3 mil.py --project={PROJECT_NAME} --dataset_root={DATASET_PATH} --label_path={LABEL_PATH} --model_path={OUTPUT_PATH} --datasets={DATASET} --input_dim={NUM_DIM} --cv_fold=1 --title={TITLE_NAME} --model=pure --baseline=dsmil --train_val --seed={SEED} --wandb')
...
with open('output.csv', 'a', newline="") as file:
    ...
    writer.writerow([args.feature_name,'-','-','-','-'])
    file.close()
os.system(f'CUDA_VISIBLE_DEVICES={CPU_ID1}, python3 eval.py --label_path={LABEL_PATH} --dataset_root={DATASET_PATH} --ckp_path={CHECKPOINT_PATH} --datasets=tct --input_dim={NUM_DIM} --model=pure --baseline=attn --seed={SEED}')
# dsmil
print('==================================\033[35mdsmil\033[0m==================================')
CHECKPOINT_PATH=f'{OUTPUT_PATH}/{PROJECT_NAME}/{FEATURE_NAME}-dsmil-{DATASET}-trainval-{SERVER}-{SEED}'
os.system(f'CUDA_VISIBLE_DEVICES={CPU_ID1}, python3 eval.py --label_path={LABEL_PATH} --dataset_root={DATASET_PATH} --ckp_path={CHECKPOINT_PATH} --datasets=tct --input_dim={NUM_DIM} --model=pure --baseline=dsmil --seed={SEED}')
# transmil
