...
#TITLE_NAME=deformable_adapter_extracted_$DLOSS-by_$VFM-$FEATURE_NAME-transmil-gc-trainval
TITLE_NAME=$FEATURE_NAME-transmil-gc-trainval
CUDA_VISIBLE_DEVICES=$GPU_ID, python3 mil.py --project=$PROJECT_NAME --dataset_root=$DATASET_PATH --label_path=$LABEL_PATH --model_path=$OUTPUT_PATH --datasets=gc --input_dim=$NUM_DIM --cv_fold=1 --title=$TITLE_NAME --model=pure --train_val --baseline=selfattn --seed=2024
