...
def main(args):
    ...
    if args.datasets.lower() == 'tct':
        test_p, test_l = parse_tct_dataset(args, 'test.csv')
    ...
    # ref: https://github.com/mahmoodlab/CLAM
        ...
        model = dsmil.MILNet(input_dim=args.input_dim,n_classes=args.n_classes).to(device)
        args.cls_alpha = 0.5
        args.cl_alpha = 0.5
    ...
    #new
    data = [auc_value*100, recall*100, float(specificity)*100, precision*100, fscore*100]
    result_dir = args.result_dir
    ouput_csv_dir = f'./eval_result/{result_dir}'
    os.makedirs(ouput_csv_dir, exist_ok=True)
    csv_save_path = os.path.join(ouput_csv_dir, args.feat_name + '.csv')
    with open(csv_save_path, 'a', newline="") as file:
        writer = csv.writer(file)
        #writer.writerow(args.model+':')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feat_name', default='test', type=str, help='Feature name of extracted patch features')
    ...
    parser.add_argument('--result_dir', type=str, default='test', help='Checkpoint path')
