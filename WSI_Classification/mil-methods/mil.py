        ...
        with amp_autocast():
            ...
            if args.model == 'mhim':
                ...
                if args.baseline == 'dsmil':
                    logit_loss = 0.5*criterion(train_logits[0].view(batch_size,-1),label) + 0.5*criterion(train_logits[1].view(batch_size,-1),label)
                else:
                    logit_loss = None
            elif args.model == 'dsmil':
                train_logits,patch_num = model(bag)
                keep_num = patch_num
                cls_loss = 0.
            elif args.model in ('clam_sb','clam_mb'):
                train_logits,cls_loss,patch_num = model(bag,label.to(torch.int64),criterion)
                keep_num = patch_num
            else:
...
if __name__ == '__main__':
    ...
    if args.train_val:
        ...
        args.val_label_path = os.path.join(args.label_path, 'test.csv')
        args.test_label_path = os.path.join(args.label_path, 'test.csv')
    else:
